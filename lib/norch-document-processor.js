...
// Allow for fully qualified paths to adapters.
var findAdapter = function(adapter){
    return ( path.dirname(adapter).length > 1 ) ? path.normalize(adapter) : "./adapters/"+adapter;
}

var adapter = require(  findAdapter( program.adapter ) ); // Finds adapter.
